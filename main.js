...
var populateDB = function(){
  dbCon.clearLocations(function(err, conIn) {
    if (err){
      console.log(err);
    } else {
      environmentCount++;
...
function getLocationData(baseIn, callback, conIn){
  environmentCount++;
        ...
        if (err){
          ...
          var checkFin = function() {
            ...
            if (countDataPoints == result.HilltopServer.DataSource.length && count == countTotal){
              dbCon.saveNewLocation(baseIn, callback, conIn);
            }
          }
